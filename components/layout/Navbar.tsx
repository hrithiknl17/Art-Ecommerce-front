...
const Navbar = () => {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (view !== 'shop' && e.target.value.length > 2) {
      setView('shop');
      setActiveCategory('All'); // Ensure we search all categories
    }
  };
  ...
  return (
    <nav className="bg-[#F8F5F1] text-[#1A1A1A] sticky top-0 z-[100] border-b border-[#1A1A1A]/10">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-b border-[#1A1A1A] absolute w-full z-50 shadow-xl">
          <div className="px-6 pt-6 pb-8 space-y-4">
            ...
            <button onClick={resetAndNavigateShop} className="block w-full text-left text-2xl font-vogue text-[#1A1A1A]">Catalogue</button>
