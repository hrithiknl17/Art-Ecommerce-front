...
const Home = () => {
  ...
  const handleGenreClick = (genreName: string) => {
    // Map friendly genre names to actual product categories
    let category = 'All';
    ...
    else category = genreName; // 'Accessories' maps directly

    setActiveCategory(category);
    setSearchQuery(''); // Clear search so the category filter works
    setView('shop');
  };
        ...
        {slides.map((slide, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4">
              ...
              <Button onClick={() => { setActiveCategory('All'); setSearchQuery(''); setView('shop'); }} className="animate-fade-in-up delay-200 bg-white text-black border-none hover:bg-[#C5A059]">
                View Collection
              </Button>
            </div>
          </div>
      ...
      <div className="bg-white py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-24 text-center">
            <Button onClick={() => { setActiveCategory('All'); setSearchQuery(''); setView('shop'); }} variant="outline" className="px-16">Enter Gallery</Button>
          </div>
        </div>
      </div>
