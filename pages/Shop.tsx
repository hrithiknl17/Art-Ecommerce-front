...
const Shop = () => {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => {
      ...
      // Search Filter (Name or Category) - Only apply if search query exists
  ...
  const handleCategoryClick = (cat: string) => {
    ...
    setSearchQuery(''); // Clear search when picking a specific category to avoid conflicts
  };
    ...
    <div className="bg-[#F8F5F1] min-h-screen pt-16 pb-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading 
          ...
          subtitle={searchQuery ? `Searching for "${searchQuery}"` : `Viewing ${activeCategory} Collection`}
        />
             ...
             <div className="flex flex-wrap justify-center gap-8">
                {categories.map(cat => (
                  <button 
                    ...
                    className={`text-xs font-bold uppercase tracking-widest transition-all ${activeCategory === cat ? 'text-[#1A1A1A] scale-110 border-b-2 border-[#C5A059]' : 'text-gray-400 hover:text-[#1A1A1A]'}`}
                  >
                    {cat}
                  </button>
                ))}
          ...
          {filteredProducts.length > 0 ? filteredProducts.map((product) => (
            <div key={product.id} className="group relative flex flex-col items-center">
              ...
              <div className="relative w-full aspect-[4/5] mb-8 p-3 bg-white frame-antique cursor-pointer" onClick={() => { setSelectedProduct(product); setView('product'); }}>
                ...
                {/* Desktop-only overlay to prevent mobile click interception */}
                <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden md:flex justify-center bg-gradient-to-t from-black/60 to-transparent pointer-events-none group-hover:pointer-events-auto">
                  <Button 
                    variant="primary" 
            ...
            <div className="col-span-full py-32 text-center">
              <p className="text-gray-400 text-xl font-vogue italic mb-4">No items found matching your criteria.</p>
