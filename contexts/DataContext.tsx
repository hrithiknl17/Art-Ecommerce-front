...
type Product = {
  ...
  category: string;
  image: string;
  images?: string[]; // Added to support gallery
  rating: number;
  stock: number;
  sales: number;
};
...
type DataContextType = {
  ...
  activeCategory: string; // Global category state
...
export const DataProvider = ({ children }: { children?: ReactNode }) => {
  const [products, setProducts] = useState(() => {
    // FORCE REFRESH: Bump version to 1.6 to fix empty state issues
    const currentVersion = '1.6';
    const savedVersion = localStorage.getItem('data_version');
    
    // If version mismatch, completely reset to initial data
    ...
    try {
      ...
      // Critical Safety Check: If data is empty or corrupted, fallback to INITIAL
  ...
  // Self-healing effect: If products ever become empty during runtime, restore them
  useEffect(() => {
  ...
  return (
    <DataContext.Provider value={{
      products, orders, customers, cart, selectedProduct, activeCategory, toast,
      setSelectedProduct, setActiveCategory, addToCart, removeFromCart, getCartTotal, clearCart,
